...
for i in range (0,lines):
    Real = randrange_float(start=-(2 ** ((N-p - 1)/2))+1, stop=(2 ** ((N-p - 1)/2))-1, step=(2 ** (-p/2)))
    Imaginary = randrange_float(start=-(2 ** ((N-p - 1)/2))+1, stop=(2 ** ((N-p - 1)/2))-1, step=(2 ** (-p/2)))
...
for row in rows:
    exp_arr_Re = np.r_[exp_arr_Re, in1_arr_Re[row]*in2_arr_Re[row] - in1_arr_Im[row]*in2_arr_Im[row] ]
    exp_arr_Im = np.r_[exp_arr_Im, in1_arr_Im[row]*in2_arr_Re[row] + in1_arr_Re[row]*in2_arr_Im[row]]
...
plt.title('REAL MUL operation with N=%d, p=%d' %(N,p))

plt.figure(3)
plt.plot(out_arr_Im,exp_arr_Im, 'bo', ms=1)
plt.xlabel('out results')
plt.ylabel('expected results')
plt.title('IMAGINARY MUL operation with N=%d, p=%d' %(N,p))
